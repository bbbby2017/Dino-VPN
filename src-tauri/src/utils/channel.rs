...
fn extract_channel_from_filename(filename: &str) -> Option<String> {
    ...
    if parts.len() > 1 {
        ...
        let ignore_list = ["setup", "rev", "portable", "x64", "x86", "arm64", "verge", "clash", "update", "installer"];
...
pub async fn apply_channel_config() {
    ...
    if current_channel.is_none() {
        logging!(info, Type::Setup, "未检测到渠道标识，按照官方默认留空");
        return;
    }
    ...
    logging!(info, Type::Setup, "检测到渠道变更: {:?} -> {}, 开始拉取对应配置...", saved_channel, channel_id);
    ...
    match tokio::time::timeout(
        ...
        Ok(Ok(mut item)) => {
            if let Ok(_) = crate::config::profiles_append_item_safe(&mut item).await {
                ...
                if let Some(uid) = &item.uid {
                    ...
                    Config::profiles().await.edit_draft(|d| d.patch_config(&switch_profiles));
                    Config::profiles().await.apply();
                    let _ = crate::config::profiles::profiles_save_file_safe().await;
                    logging!(info, Type::Setup, "配置已生效为默认选项");
                    
                    // MUST trigger core engine reload and UI refresh, else nodes are empty natively
