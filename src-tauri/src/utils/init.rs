...
async fn init_dns_config() -> Result<()> {
    ...
    if !dns_path.exists() {
        ...
        help::save_yaml(&dns_path, &default_dns_config, Some("# DinoVPN DNS Config")).await?;
    }
...
pub fn init_scheme() -> Result<()> {
    ...
    clash.set_value("", &"DinoVPN")?;
    clash.set_value("URL Protocol", &"DinoVPN URL Scheme Protocol")?;
